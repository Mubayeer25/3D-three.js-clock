...
renderer.clear();
//renderer.setSize(window.innerWidth, window.innerHeight);
renderer.setClearColor('cyan');    // set background color
...
loader1.load('https://images.pexels.com/photos/1205301/pexels-photo-1205301.jpeg' , function(texture1)
            {
...
const texture = new THREE.TextureLoader().load('./image/crate.jpg');
...
const latheMat = new THREE.MeshBasicMaterial( { color: "brown" } );
...
const blob = new THREE.Mesh(blobGeo, new THREE.MeshBasicMaterial( {color: 'yellow'} ));
cylinder.add(blob);
...
// create hourHand for Central  Time (CT)
const hourGeoCET = new THREE.SphereGeometry( clockSettings.radiusHour, 10, 10 );
hourGeoCET.scale(0.03, 0.03, 1);
const hourHandCET = new THREE.Mesh(hourGeoCET, new THREE.MeshPhongMaterial({color: "magenta"}));
blob.add(hourHandCET);


// create minuteHand for Central  Time (CT)
const minuteGeoCET = new THREE.SphereGeometry( clockSettings.radiusMin, 10, 10 );
minuteGeoCET.scale(0.03, 0.03, 1);
const minuteHandCET = new THREE.Mesh(minuteGeoCET, new THREE.MeshPhongMaterial({color: "orange"}));
blob.add(minuteHandCET);


// create secondHand for Central  Time (CT)
const secGeoCET = new THREE.SphereGeometry( clockSettings.radiusSec, 10, 10 );
secGeoCET.scale(0.03, 0.03, 1);
const secHandCET = new THREE.Mesh(secGeoCET, new THREE.MeshPhongMaterial({color: "green"}));
blob.add(secHandCET);


//**** For IST ****//

// create hourHand for  Standard Time (ST)
const hourGeoIST = new THREE.SphereGeometry( clockSettings.radiusHour, 10, 10 );
hourGeoIST.scale(0.03, 0.03, 1);
const hourHandIST = new THREE.Mesh(hourGeoIST, new THREE.MeshPhongMaterial({color: "white"}));
blob.add(hourHandIST);


// create minuteHand for  Standard Time (ST)
const minuteGeoIST = new THREE.SphereGeometry( clockSettings.radiusMin, 10, 10 );
minuteGeoIST.scale(0.03, 0.03, 1);
const minuteHandIST = new THREE.Mesh(minuteGeoIST, new THREE.MeshPhongMaterial({color: "white"}));
blob.add(minuteHandIST);


// create secondHand for Indian Standard Time (ST)
const secGeoIST = new THREE.SphereGeometry( clockSettings.radiusSec, 10, 10 );
secGeoIST.scale(0.03, 0.03, 1);
const secHandIST = new THREE.Mesh(secGeoIST, new THREE.MeshPhongMaterial({color: "white"}));
blob.add(secHandIST);
	...
	if(i==clockSettings.frontClock){
		...
		for(let j=0; j<clockSettings.numOfMinutes; j++){
			...
			/* check for bigger ticks in every five minutes */	
				...
				createLineGeometry(cylinder, clockSettings.lineWidth, clockSettings.lineHeight, "violet", frontAngle, frontSide); // create box geomentry for every five minutes
  ...
  /* check if the clock is back sided i.e. '1' */
	  ...
	  for(let k=0; k<clockSettings.numOfMinutes; k++){
			...
			/* check for bigger ticks in every five minutes */
				...
				createLineGeometry(cylinder, clockSettings.lineWidth, clockSettings.lineHeight, "green", backAngle, backSide); // create box geomentry for every five minutes
